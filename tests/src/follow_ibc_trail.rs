...
pub async fn follow_trail(chain1: &Daemon, chain2: &Daemon, tx_hash: String) -> Result<()> {
    ...
    let connection = send_packet_events[0]
        .get_first_attribute_value("packet_connection")
        .unwrap();
    let dest_port = send_packet_events[0]
        .get_first_attribute_value("packet_dst_port")
        .unwrap();
    let dest_channel = send_packet_events[0]
        .get_first_attribute_value("packet_dst_channel")
        .unwrap();
    let sequence = send_packet_events[0]
        .get_first_attribute_value("packet_sequence")
        .unwrap();
    let packet_data = send_packet_events[0]
    ...
    let events_string = vec![
        format!("recv_packet.packet_connection='{}'", connection),
        format!("recv_packet.packet_dst_port='{}'", dest_port),
        format!("recv_packet.packet_dst_channel='{}'", dest_channel),
    ...
    let txs = Node::new(chain2.channel())
        .find_tx_by_events(events_string, None, None)
        .await
        .unwrap();
    ...
    let recv_packet_sequence = received_tx.get_events("write_acknowledgement")[0]
        .get_first_attribute_value("packet_sequence")
        .unwrap();
    ...
    let ack_events_string = vec![
        format!("acknowledge_packet.packet_connection='{}'", connection),
        format!("acknowledge_packet.packet_dst_port='{}'", dest_port),
        format!("acknowledge_packet.packet_dst_channel='{}'", dest_channel),
        format!("acknowledge_packet.packet_sequence='{}'", sequence),
    ];
