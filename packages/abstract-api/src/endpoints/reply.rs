...
mod tests {
    ...
    use cosmwasm_std::{testing::{mock_dependencies, mock_env}, Reply, SubMsgResponse};
    ...
    fn endpoint() -> ApiMockResult {
        ...
        let reply_msg = Reply{id: 1, result: cosmwasm_std::SubMsgResult::Ok(SubMsgResponse{events: vec![], data: Some("test_reply".as_bytes().into())})};
    ...
    fn no_matching_id() -> ApiMockResult {
        ...
        let mut deps = mock_dependencies();
        deps.querier = abstract_testing::mock_querier();
        let reply_msg = Reply{id: 0, result: cosmwasm_std::SubMsgResult::Ok(SubMsgResponse{events: vec![], data: Some("test_reply".as_bytes().into())})};
        let res = reply(deps.as_mut(), env, reply_msg);
        assert_that!(res).is_err().is_equal_to(&AbstractSdkError::MissingHandler { endpoint: "reply with id 0".into() }.into());
        Ok(())
    }
}
