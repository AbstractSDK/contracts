...
use abstract_interface::{api::ApiRequestMsg, objects::module::ModuleId};
...
impl<'a, T: ApiInterface> Api<'a, T> {
    ...
    pub fn request<M: Serialize + Into<abstract_interface::api::ExecuteMsg<M, Empty>>>(
        &self,
        api_id: ModuleId,
        message: M,
        ...
        let api_msg = abstract_interface::api::ExecuteMsg::<_>::Module(ApiRequestMsg::new(
    ...
    pub fn query<Q: Serialize, R: DeserializeOwned>(
        &self,
        api_id: ModuleId,
        message: impl Into<abstract_interface::api::QueryMsg<Q>>,
    ) -> AbstractSdkResult<R> {
        let api_msg: abstract_interface::api::QueryMsg<Q> = message.into();
        let modules = self.base.modules(self.deps);
        let api_address = modules.module_address(api_id)?;
        self.deps
            .querier
