...
// use abstract_os::objects::{UncheckedContractEntry, UncheckedDexPoolEntry};
//
...
//     }
//
//     pub fn update_dex_pools(&self) -> Result<(), BootError> {
//         let path = env::var("MEMORY_DEX_POOLS")?;
//
//         let file = File::open(&path).expect(&format!("file should be present at {}", &path));
//         let json: serde_json::Value = from_reader(file)?;
//         let chain_id = self.0.chain().state.chain.chain_id;
...
//             Some(contracts_value) => {
//                 let contracts = contracts_value.as_object().unwrap();
//                 let to_add: Vec<(UncheckedDexPoolEntry, UncheckedPool)> = contracts
//                     .iter()
//                     .map(|(name, value)| {
//                         let id = value.as_str().unwrap().to_owned();
//                         let key = UncheckedDexPoolEntry::try_from(name.clone()).unwrap();
//                         (key, id)
//                     })
//                     .collect();
//                 let mut i = 0;
