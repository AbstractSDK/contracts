...
impl AbstractAccount<Daemon> {
    ...
    pub fn upload_and_register_if_needed(&self, version_control: &VersionControl<Daemon>) -> Result<(), AbstractInterfaceError> {
        let mut modules_to_register = Vec::with_capacity(2);

        if let Some(_) = self.manager.upload_if_needed()? {
            modules_to_register.push((self.manager.as_instance(), ::manager::contract::CONTRACT_VERSION.to_string()));
        };

        if let Some(_) = self.proxy.upload_if_needed()?{
            modules_to_register.push((self.proxy.as_instance(), ::proxy::contract::CONTRACT_VERSION.to_string()));
        }; 
