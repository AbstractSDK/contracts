...
pub fn diff<K, V>(scraped_entries: HashMap<K, V>, on_chain_entries: HashMap<K, V>) -> Result<(HashSet<K>,HashMap<K, V>), AbstractInterfaceError> 
        where K: Eq + Hash + Clone,
             V: Clone + PartialEq
{
    let union_keys = get_union_keys(&scraped_entries, &on_chain_entries);
    Ok(get_changes(&union_keys, &scraped_entries, &on_chain_entries))
}
