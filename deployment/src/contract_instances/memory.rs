...
use serde::Serialize;
use terra_rust_api::{client::{wasm::Wasm, tx_types::TXResultSync}, errors::TerraRustAPIError};
...
impl Memory {
    async fn execute<C: Signing + Context>(&mut self, sender: Sender<C>, exec_msg: ExecuteMsg,) -> Result<TXResultSync, TerraRustAPIError>{
        self.instance.interface.execute_msg = exec_msg; 
        self.instance.execute().await?;
