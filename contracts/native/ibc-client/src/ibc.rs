...
use abstract_sdk::core::{
    ...
    ibc_client::{
        state::{ACCOUNTS, CHANNELS, CHAIN_HOSTS},
        CallbackInfo,
    },
...
fn acknowledge_who_am_i(
    ...
    let registered_host = CHAIN_HOSTS.load(deps.storage, &chain).map_err(|_| IbcClientError::UnregisteredChain(chain.to_string()))?;
    let counterparty_host = port_id
    .strip_prefix("wasm.")
