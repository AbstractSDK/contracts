...
fn test_registered_client(){
	...
	// Register
	execute(deps.as_mut(), mock_env(), info, abstract_core::ibc_host::ExecuteMsg::RegisterChainClient { chain_id: "juno".to_string(), client: "juno-client".to_string() }).unwrap();

	// Query
	let client_name = query(deps.as_ref(), mock_env(), QueryMsg::AssociatedClient { chain: "juno".to_string() }).unwrap();
	...
	receive_who_am_i(deps.as_mut(), "channel-1".to_string(), IbcEndpoint{
		channel_id: "channel-1".to_string(),
		port_id: "wasm.juno-client".to_string(),
	}, ChainName::from("juno"), ChainName::from("osmosis-2")).unwrap();
