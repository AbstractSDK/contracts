...
use abstract_boot::{Abstract, Manager, OS, AbstractBootError};
...
use boot_core::{instantiate_default_mock_env, Addr, Deploy, Empty, Mock, ContractInstance, BootError};
use common::mock_modules::*;
use common::{create_default_os, init_abstract_env, init_mock_api, AResult, TEST_COIN};
use speculoos::prelude::*;

fn install_module_version(manager: &Manager<Mock>,abstr: &Abstract<Mock>, module: &str, version: &str) -> anyhow::Result<String> {
    manager
        .install_module_version(
            module,
...
fn install_app_successful() -> AResult {
    ...
    assert_that!(res.unwrap_err().root_cause().to_string()).contains("module tester:mock-api1 is a dependency of tester:mock-app1 and is not installed.");
    ...
    let res = install_module_version(manager, &abstr, MOCK_APP1_ID, V1);
    assert_that!(&res).is_err();
    assert_that!(res.unwrap_err().root_cause().to_string()).contains("module tester:mock-api2 is a dependency of tester:mock-app1 and is not installed.");
