...
pub fn receive_cw20(
    ...
    match from_binary(&cw20_msg.msg)? {
        DepositHookMsg::Pay { os_id } => {
            ...
            try_pay(add_on, deps, env, msg_info, asset, os_id)
        }
    }
}
...
pub fn claim_subscriber_emissions(
    ...
    if !asset.amount.is_zero() {
        add_on
            .execute_on_proxy(deps, vec![asset.transfer_msg(subscriber_proxy_address)?])
            .map_err(From::from)
    } else {
        Ok(Response::new())
    }
...
pub fn try_claim_compensation(
    ...
    if msgs.is_empty() {
        ...
        Ok(add_on
            .execute_on_proxy(deps.as_ref(), msgs)?
...
fn expired_sub_msgs(
    ...
    if subscriber.expiration_block <= env.block.height {
        let mut resp = claim_subscriber_emissions(add_on, deps, env, os_id)?;
