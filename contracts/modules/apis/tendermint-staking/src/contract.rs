...
pub fn handle_api_request( deps: DepsMut,
    _env: Env,
    _info: MessageInfo,
    ...
    match msg {
        ExecuteMsg::Delegate { validator, amount } => api.os_execute(deps.as_ref(),vec![delegate_to(&deps.querier, &validator, amount.u128())?]),
        ...
        ExecuteMsg::UndelegateAll { } => api.os_execute(deps.as_ref(),undelegate_all(&deps.querier, &api.request_destination)?),

        ExecuteMsg::Redelegate { source_validator, destination_validator, amount } => {
            let redelegate_msg = match amount {
                Some(amount) => redelegate(&deps.querier, &source_validator, &destination_validator,amount.u128())?,
                None => redelegate_all(&deps.querier, &source_validator, &destination_validator, &api.request_destination)?,
            };
            api.os_execute(deps.as_ref(),vec![redelegate_msg])
        },
        ExecuteMsg::SetWithdrawAddress { new_withdraw_address } => api.os_execute(deps.as_ref(),vec![update_withdraw_address(deps.api, &new_withdraw_address)?]),
        ExecuteMsg::WithdrawDelegatorReward { validator } => api.os_execute(deps.as_ref(),vec![withdraw_rewards(&validator)]),
        ExecuteMsg::WithdrawAllRewards { } => api.os_execute(deps.as_ref(),withdraw_all_rewards(&deps.querier, &api.request_destination)?),
    }.map_err(From::from)
